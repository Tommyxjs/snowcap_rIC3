...
impl ExampleNetwork for SimpleNet {
    ...
    fn initial_config(net: &Network, variant: usize) -> Config {
        ...
        c.add(IgpLinkWeight { source: r4, target: r5, weight: 1.0 }).unwrap();
        c.add(IgpLinkWeight { source: r1, target: e1, weight: 10.0 }).unwrap();
        c.add(IgpLinkWeight { source: r5, target: e4, weight: 1.0 }).unwrap();
        ...
        c.add(IgpLinkWeight { target: r4, source: r5, weight: 1.0 }).unwrap();
        c.add(IgpLinkWeight { target: r1, source: e1, weight: 10.0 }).unwrap();
        c.add(IgpLinkWeight { target: r5, source: e4, weight: 1.0 }).unwrap();

        c.add(BgpSession { source: r1, target: e1, session_type: EBgp }).unwrap();
        c.add(BgpSession { source: r5, target: e4, session_type: EBgp }).unwrap();
        c.add(BgpSession { source: r2, target: r1, session_type: IBgpClient }).unwrap();
        c.add(BgpSession { source: r3, target: r1, session_type: IBgpClient }).unwrap();
        c.add(BgpSession { source: r2, target: r3, session_type: IBgpPeer }).unwrap();
        c.add(BgpSession { source: r4, target: r1, session_type: IBgpClient }).unwrap();
        c.add(BgpSession { source: r4, target: r5, session_type: IBgpClient }).unwrap();
    ...
    fn final_config(net: &Network, variant: usize) -> Config {
        ...
        c.add(BgpSession { source: r2, target: r3, session_type: IBgpPeer }).unwrap();
        c.add(BgpSession { source: r4, target: r1, session_type: IBgpClient }).unwrap();
        c.add(BgpSession { source: r2, target: r4, session_type: IBgpClient }).unwrap();
        c.add(BgpSession { source: r3, target: r5, session_type: IBgpClient }).unwrap();
